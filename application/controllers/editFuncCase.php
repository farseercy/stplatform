...
$_SESSION['postParam']="";
$_SESSION['testResult']="";
class EditFuncCase extends MY_Controller {
	...
	public function exchangeLevel($module,$caseid="")
    {
    	$this->show($module,$caseid);
    }
    
    private function show($module,$caseid="")
    {   
    	$this->platInfoArray = $this->config->item('plat_info_array');
    	$this->platInfoArray['curModule']=$module;
		$this->load->model('case_model');
		$interface=$this->case_model->get_interfaces_by_funcID('tb_func',$caseid);
		$gets=$this->case_model->get_getParams_key('tb_interface',$interface);
		$getParams=@split(";",$gets);
		/*$getParams['key']=@split(";",$gets);
		$gets=$this->case_model->get_getParams_value('tb_func',$caseid);
		$getParams['value']=@split("&",$gets);
		var_dump($getParams);*/
		$posts=$this->case_model->get_postParams('tb_interface',$interface);
		$postParams=@split(";",$posts);
		...
		$this->cismarty->assign('getParams', $getParams); 
		$this->cismarty->assign('postParams', $postParams); 
		$this->cismarty->assign('caseid', $caseid);       
        $this->cismarty->display(APPPATH . '/views/module/editCase.tpl');   
    }
